...
#pragma region adding and renderable objects to the scene
	...
	Plain* redPlain = new Plain(Vector((float)0,(float)1.22, (float)55.12), red, Vector((float)-2, (float)-2, (float)1), Vector((float)2, (float)-2, (float)1), Vector((float)2, (float)2, (float)100), Vector((float)-2, (float)2, (float)100));
...
#pragma endregion
	...
	static float distance = (float)(renderables->at(guiObjectIndex)->GetPos() - cameraPosition).getMagnitude();
	float fov = 30;
	...
	Camera* secondaryCamera = new Camera(cameraPosition, camLook, Vector(0, 100000000, 0), fov, width/height);
...
#pragma region TODO: add rendering to texture for the rendermethod instead of glDrawPixels
	...
	GLuint textureHandle = 0;//handle to the texture object
	glGenTextures(1, &textureHandle);//generates a texture
	...
	glBindTexture(GL_TEXTURE_2D, textureHandle);//binds the texture to the handle
